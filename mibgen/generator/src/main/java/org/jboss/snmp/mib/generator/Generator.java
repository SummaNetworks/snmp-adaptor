...
public class Generator {
	...
	private ArrayList<MappedAttribute> maList; // list of Mapped Attributes we
												// care about
	private ArrayList<Mapping> nmList; // list of notification mappings we care
										// about
	...
	public Generator(String outputResName, String moduleName,
			ArrayList<MappedAttribute> maList, AttributeMappings mbList,
	...
	private void writeMibHeader(PrintWriter out) {
		...
		out.println();
		// maybe add more here, if not we don't need this to be a seperate
		// method.
	}
	...
	private void createEntries() {
		if (maList != null) {
			...
			System.out
					.println("Attribute parsing was skipped. No attribute MIB definitions written.");
		}
		if (mbList != null) {
			for (ManagedBean mb : mbList) {
				// we only want to make a table if the MBean's name is a
				// wildcard.
			...
			System.out
					.println("Attribute parsing was skipped. No list of MBeans is available.");
		}
		if (nmList != null) {
			...
			System.out
					.println("Notification parsing was skipped. No notification MIB definitions written.");
		}
	}
	...
	private class MIBObject extends MIBEntity {
		...
		private boolean rowEntry; // internally used to avoid adding an OIDDef
									// for a member of an entry
		...
		MIBObject(String name, VarBind vb) throws NotEnoughInformationException {
			...
			// wiki says: if entry does not exist, it will be created.... if
			// null is passed,
			// definition-name is ignored from var-binding!
		...
		MIBObject(MappedAttribute ma, boolean rowEntry)
			...
			// Otherwise we cannot know it, use
			// the general OCTET STRING type.
			this.syntax = (ma.getSnmpType() != null) ? ma.getSnmpType()
					: "OCTET STRING (SIZE(0..255))";

			// there are more values possible here for an MIB, but not for a JMX
			// attr; we only
			...
			// perhaps have this as an optional attribute in the attributes.xml?
			// Left as blank
			// because there is no way to query the server about a given
			// attribute
			this.description = (ma.getSnmpDesc() != null) ? ma.getSnmpDesc()
					: "";

			String[] temp = ma.getOid().split("\\."); // this will contain the
														// full numerical OID.
			// We need the last element in the array to be the registered OID,
			// so we can put the correct
			// name in the output MIB
			this.objectId = temp[temp.length - 1];
			this.fullOid = ma.getOidPrefix() + "." + this.objectId;
			// check if there is already an OID definition for this prefix. If
			// there is make this MIBObject's
			// oidDef reflect that
			if (this.rowEntry)
	...
	private class MIBNotification extends MIBEntity {
		...
		public MIBNotification(Mapping mp) throws NotEnoughInformationException {
			...
			if (this.name == null) {
				throw new NotEnoughInformationException("The notification "
						+ mp.getNotificationType()
						+ " has no valid name for the MIB.");
			}
			this.status = (mp.getStatus() != null) ? mp.getStatus() : "current";
			this.description = (mp.getDesc() != null) ? mp.getDesc() : "";
			ArrayList<VarBind> vbList = (ArrayList<VarBind>) mp
					.getVarBindList().getVarBindList();
			setObjects(vbList);
			// the OID of a v2 trap = <enterpriseid>.0.<specificid>
			// for predefined traps, see RFC1907 / 3413 / 3418 /
			// http://www.oid-info.com/get/1.3.6.1.6.3.13
			String oidPrefix = mp.getEnterprise() + "."
					+ String.valueOf(mp.getGeneric());
		...
		public void setObjects(ArrayList<VarBind> vbList)
				throws NotEnoughInformationException {
			// there are special conditions in this section that we need to
			// account for, and
			...
			// have all the oids. compare these to the MIBObject's full OID. if
			// it fails,
			...
			nextOid: while (oidIt.hasNext()) {
				...
				// put UNKNOWNOBJECT here instead.
				// this.objects.add("UNKNOWNOBJECT");
				throw new NotEnoughInformationException("The notification "
						+ this.name
						+ " contains an OID that does not exist in the MIB.");
			}
		}

		// if we found a tag that matches n:.*, we create a new MIBObject so
		// that this object
		...
		public void createNotifPayloadMibo(VarBind vb)
			...
			String name = "jbossJmxNotification"
					+ tag[tag.length - 1].substring(0, 1).toUpperCase()
					+ tag[tag.length - 1].substring(1);
	...
	private class OIDDef {
		...
		private String definition; // the OID with '.' replaced by ' ', ready to
									// be output into the MIB
	...
	private class MIBTable extends MIBEntity {
		...
		private MIBTableRow row; // the row created from information contained
									// in
									// this MIBTable
		...
		public MIBTable(MappedAttribute ma)
				throws NotEnoughInformationException {
			this.tablePrefix = ma.getName().substring(0, 1).toLowerCase()
			...
			this.syntax = this.rowName.substring(0, 1).toUpperCase()
					+ this.rowName.substring(1);
			this.status = (ma.getStatus() != null) ? ma.getStatus() : "current";
			setOidDef(ma.getOidPrefix(), ma.getOidDefName());
			...
			if (temp.length == 3) { // .x.y, .x == TableOid, .y = RowOid
				...
				row = new MIBTableRow(this.name, this.tablePrefix,
						this.rowName, temp[2], ma.getMode());
			} else {
				this.objectId = temp[temp.length - 1];
				row = new MIBTableRow(this.name, this.tablePrefix,
						this.rowName, "1", ma.getMode());
			}
		...
		public MIBTable(ManagedBean mb) throws NotEnoughInformationException {
			...
			if (this.tablePrefix == null) {
				throw new NotEnoughInformationException(
						"The mbean "
								+ mb.getName()
								+ " has no table-name attribute defined. MIB Generation failed.");
			}
			this.name = this.tablePrefix + "Table";
			this.maxAccess = "not-accessible";
			...
			if (oid == null) {
				throw new NotEnoughInformationException(
						"The mbean "
								+ mb.getName()
								+ " has no oid-prefix attribute defined. MIB Generation failed.");
			}
			String oidPrefix = "";
			...
			row = new MIBTableRow(this.name, this.tablePrefix, this.rowName,
					temp[temp.length - 1],
					(ArrayList<MappedAttribute>) mb.getAttributes());
		}
		...
		public String toString() {
			...
			buf.append(this.name).append(" OBJECT-TYPE").append("\n")
					.append("\tSYNTAX\tSEQUENCE OF ").append(this.syntax)
					.append("\n").append("\tMAX-ACCESS\t")
					.append(this.maxAccess).append("\n").append("\tSTATUS\t")
					.append(this.status).append("\n")
					.append("\tDESCRIPTION\n\t\t")
					.append("\"" + this.description + "\"").append("\n")
					.append("::= { ")
					.append(this.oidDef + " " + this.objectId + " }\n\n")
					.append(row);
			return buf.toString();
		}
	...
	private class MIBTableRow extends MIBEntity {
		...
		MIBTableRow(String tableName, String tablePrefix, String rowName,
				String rowOid, String mode)
				throws NotEnoughInformationException {
			this.tableName = tableName;
			this.tablePrefix = tablePrefix;
			this.name = rowName;
			this.objectId = rowOid;
			this.typeName = this.name.substring(0, 1).toUpperCase()
		...
		MIBTableRow(String tableName, String tablePrefix, String rowName,
				String rowOid, ArrayList<MappedAttribute> attrList)
				throws NotEnoughInformationException {
			this.tableName = tableName;
			this.tablePrefix = tablePrefix;
			this.name = rowName;
			...
			this.syntax = rowName;
			this.maxAccess = "not-accessible";
			this.status = "current"; // doesn't make sense to define a table
										// that is not current
			this.description = "";
		...
		private void setRowObjects(ArrayList<MappedAttribute> attrList)
				throws NotEnoughInformationException {
			this.rowObjects = new ArrayList<MIBObject>(1);
			// check if there's an invalid OID in the attrList, ie: one defined
			// with a .1 oid
			for (MappedAttribute ma : attrList) {
				if (ma.getOid().equals(".1")) {
					System.out
							.println("The attribute '"
									+ ma.getName()
									+ "' has an OID of 1. This value is reserved for the index in a table. MIB Generation failed.");
					System.exit(1);
				}
			}
			...
			for (MappedAttribute ma : attrList) {
				...
				ma.setName(this.tablePrefix
						+ ma.getName().substring(0, 1).toUpperCase()
						+ ma.getName().substring(1));
		...
		private String writeRowDefinition() {
			...
			for (int i = 0; i < rowObjects.size(); i++) {
				if (i == rowObjects.size() - 1)
					buf.append("\t" + rowObjects.get(i).getName() + "\t"
							+ rowObjects.get(i).getSyntax());
				else
					buf.append("\t" + rowObjects.get(i).getName() + "\t"
							+ rowObjects.get(i).getSyntax() + ",\n");
			}
		...
		public String toString() {
			...
			buf.append(this.name)
					...
					.append("INDEX\t{\n\t\tIMPLIED " + this.indexName
							+ "\n\t\t}\n").append("::= { ")
					.append(this.tableName).append(" " + this.objectId)
					.append(" }\n\n");
	...
	private abstract class MIBEntity {
		...
		public void setOidDef(String oidPrefix, String oidDefName)
			...
			if (oidDefMap.containsKey(oidPrefix)) {
				...
				} else { // everything failed. the attributes.xml doesn't
							// specify a name for the oid definition to be used,
							// nor do we already know about it.
					throw new NotEnoughInformationException(
							"There was no definition-name for oid-prefix: "
									+ oidPrefix);
				}
			}
		}
		...
		public boolean equals(Object o) {
			...
			if (this.objectId != null ? !this.objectId.equals(test.objectId)
					: test.objectId != null)
				return false;

			if (this.oidDef != null ? !this.oidDef.equals(test.oidDef)
					: test.oidDef != null)
				return false;
